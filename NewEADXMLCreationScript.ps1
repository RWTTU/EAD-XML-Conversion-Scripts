...
function codedDate($i, $minyear, $maxyear) {

   
    # 1 October-December, 2001
    if ($i -match $regex_case1 ) {
        $year = $matches[3]; $month = Convert-Date $matches[1]
    ...
    # 10 c. 1945-1947
    elseif ($i -match $regex_case10 ) {
        $year = $matches[1]; $year2 = $matches[2];
        return "${year}/${year2}"
    }
    # 11 1945 and c. 1945
    elseif ($i -match $regex_case11  ) {
        $year = $matches[1];
        return "${year}" #>> $outpath
    }
    # 12 1942, 1045, 1945-1947
    elseif ($i -match $regex_case12 ) {
        $str2 = $matches[1];  
        $str2 = $str2 -replace ",\s*|\s*-\s*" , ",";
        $str3 = $str2.Split(",");
        $min = $str3 | measure -Minimum -Maximum;
