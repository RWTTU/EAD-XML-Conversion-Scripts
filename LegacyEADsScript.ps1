...
$maxyear = 0
     ...
     foreach($i in $csv1){
       ...
       if($i -match "([a-zA-Z]+)\s*,?\s*\b(\d{1,2})?(?:[nN][dD]|[sS][tT]|[rR][dD]|[tT][hH])?\b\s*,?\s*(\d{4})?(\s*.{1,2}\b\s*([a-zA-Z]+)\s*,?\s*\b(\d{1,2})?(?:[nN][dD]|[sS][tT]|[rR][dD]|[tT][hH])?\b\s*,?\s*(\d{4})?)" -and $i -notlike "*undated*" -and $i -notlike "*a.m.*" -and $i -notlike "*p.m.*"){
        ...
        #October, 2001 Done
        if($i -match "^[a-zA-Z]+,?\s*(\d{4})$" -and   $i -notlike "Spring*" -and   $i -notlike "Fall*" -and   $i -notlike "Summer*" -and   $i -notlike "Winter*" -and   $i -inotlike "circa*"){
...
$maxyear = $args[2]
    ...
    #October, 2001
    elseif($i -match "^[a-zA-Z]+,?\s*(\d{4})$" -and   $i -notlike "Spring*" -and   $i -notlike "Fall*" -and   $i -notlike "Summer*" -and   $i -notlike "Winter*"  -and   $i -inotlike "circa*" -and $i -ne "sfwxyswzFXSXyfqys"){
